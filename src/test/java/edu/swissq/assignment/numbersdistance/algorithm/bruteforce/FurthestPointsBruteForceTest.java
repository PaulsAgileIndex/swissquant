...
public class FurthestPointsBruteForceTest {
	
	private static Map<Integer, Point> pointUniverse;
	private static FurthestPointsBruteForce algorithm;
	...
	public static void setUpOnce() {
		...
		algorithm = new FurthestPointsBruteForce();
	}
	
	@Test
	public void testGetFurthestPoints() {
		List<Point> resultList = algorithm.getFurthestPoints(pointUniverse, new Point(7, 7), 4);
		assertThat(resultList).isNotNull().isNotEmpty().hasSize(4);
		for (Point furthestPoint : furtherPoints) {
			boolean match = false;
			for (Point resultPoint : resultList) {
				if(furthestPoint.compareTo(resultPoint) == 0) {
					match = true;
				}
			}
